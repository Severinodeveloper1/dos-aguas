...
@section('content')
    ...
    <main class="max-w-container-max mx-auto px-margin-edge py-20 font-body">
            ...
            <div class="lg:col-span-6 space-y-6">
                ...
                <div class="border-t border-outline-variant/10 pt-6 grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
                    <div>
                        ...
                        <span class="font-label-caps text-[9px] tracking-wider text-outline font-bold">{{ __('messages.about.trazable') }}</span>
                    </div>
                    <div>
                        <span class="block font-headline text-2xl text-primary font-bold">{{ __('messages.about.origen') }}</span>
                        <span class="font-label-caps text-[9px] tracking-wider text-outline font-bold">{{ __('messages.about.solido') }}</span>
                    </div>
                    <div>
                        <span class="block font-headline text-2xl text-primary font-bold">{{ __('messages.about.hecho') }}</span>
                        <span class="font-label-caps text-[9px] tracking-wider text-outline font-bold">{{ __('messages.about.a_mano') }}</span>
                    </div>
                </div>
            </div>
        ...
        @php
            ...
            if (empty($galleryImagesRow1)) $galleryImagesRow1 = $photosResolved;
            if (empty($galleryImagesRow2)) $galleryImagesRow2 = $photosResolved;
            if (empty($galleryImagesRow3)) $galleryImagesRow3 = $photosResolved;
        @endphp
        ...
        <section class="border-t border-outline-variant/10 pt-20" x-data="{ activeImage: null }">
                    ...
                    <div class="marquee-ltr-container flex gap-6">
                        @foreach($galleryImagesRow1 as $img)
                            <div class="w-72 aspect-[4/3] bg-surface-container border border-outline-variant/10 overflow-hidden cursor-pointer flex-shrink-0"
                                ...
                                <img src="{{ $img }}" alt="Gallery 1" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"/>
                            </div>
                        @endforeach
                        @foreach($galleryImagesRow1 as $img)
                            <div class="w-72 aspect-[4/3] bg-surface-container border border-outline-variant/10 overflow-hidden cursor-pointer flex-shrink-0"
                                 @click="activeImage = '{{ $img }}'">
                                <img src="{{ $img }}" alt="Gallery 1 duplicate" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"/>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="overflow-hidden w-full flex">
                    <div class="marquee-rtl-container flex gap-6">
                        @foreach($galleryImagesRow2 as $img)
                            <div class="w-72 aspect-[4/3] bg-surface-container border border-outline-variant/10 overflow-hidden cursor-pointer flex-shrink-0"
                                 @click="activeImage = '{{ $img }}'">
                                <img src="{{ $img }}" alt="Gallery 2" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"/>
                            </div>
                        @endforeach
                        @foreach($galleryImagesRow2 as $img)
                            <div class="w-72 aspect-[4/3] bg-surface-container border border-outline-variant/10 overflow-hidden cursor-pointer flex-shrink-0"
                                 @click="activeImage = '{{ $img }}'">
                                <img src="{{ $img }}" alt="Gallery 2 duplicate" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"/>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="overflow-hidden w-full flex">
                    <div class="marquee-ltr-container flex gap-6">
                        @foreach($galleryImagesRow3 as $img)
                            <div class="w-72 aspect-[4/3] bg-surface-container border border-outline-variant/10 overflow-hidden cursor-pointer flex-shrink-0"
                                 @click="activeImage = '{{ $img }}'">
                                <img src="{{ $img }}" alt="Gallery 3" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"/>
                            </div>
                        @endforeach
                        @foreach($galleryImagesRow3 as $img)
                            <div class="w-72 aspect-[4/3] bg-surface-container border border-outline-variant/10 overflow-hidden cursor-pointer flex-shrink-0"
                                 @click="activeImage = '{{ $img }}'">
                                <img src="{{ $img }}" alt="Gallery 3 duplicate" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105"/>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div x-show="activeImage" 
                ...
                <div class="relative bg-[#161616] border border-outline-variant/20 max-w-4xl w-full p-4 flex flex-col gap-4">
                    <button class="absolute -top-12 right-0 text-on-surface hover:text-primary flex items-center gap-2 font-label-caps text-xs tracking-wider" 
