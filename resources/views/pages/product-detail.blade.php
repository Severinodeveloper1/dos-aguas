    ...
    <main class="max-w-container-max mx-auto px-margin-edge py-20 font-body">
                    ...
                    <div class="grid grid-cols-5 gap-4">
                        @foreach ($product->images as $img)
                            ...
                            <div class="aspect-square bg-surface-container border border-outline-variant/10 hover:border-primary/50 transition-colors cursor-pointer"
                                ...
                                <img class="w-full h-full object-cover" src="{{ $thumbUrl }}"
            ...
            <div class="lg:col-span-5 lg:pl-8 flex flex-col gap-8">
                ...
                <div class="flex flex-col gap-2">
                    <label for="variant-select"
                        class="font-label-caps text-[10px] tracking-widest text-outline uppercase font-bold">
                        {{ app()->getLocale() == 'es' ? 'Presentación' : 'Presentation' }}
                    </label>
                    <select id="variant-select" x-model="selectedId"
                ...
                <!-- Stock status -->
                <div
                    class="border-t border-b border-outline-variant/10 py-4 flex justify-between items-center text-xs font-body">
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div x-show="message" x-transition class="p-3 text-xs font-body font-bold border"
                        :class="messageType === 'success' ? 'bg-leaf-green/10 border-leaf-green/30 text-leaf-green' :
                    ...
                    <div class="flex items-center h-14 w-full gap-4">
                        <!-- Qty adjustment -->
                        <div
                            class="flex border border-outline-variant/30 h-full items-center px-4 gap-6 font-body select-none">
                            <button type="button" class="hover:text-primary font-bold p-1"
                                @click="if(qty > 1) qty--">-</button>
                            <span class="font-bold text-center w-6 text-sm" x-text="qty"></span>
                            <button type="button" class="hover:text-primary font-bold p-1"
                                @click="if(qty < selectedStock) qty++">+</button>
                        </div>

                        <!-- Add to Cart CTA -->
                        <button type="button" @click="addToCart()" :disabled="selectedStock <= 0 || loading"
                        ...
                        <div class="border-b border-outline-variant/10 pb-4">
                            <button @click="activeTab = activeTab === 'notes' ? null : 'notes'"
                                ...
                                <span class="material-symbols-outlined text-[16px] transition-transform"
                                    :class="activeTab === 'notes' ? 'rotate-180 text-primary' : ''">expand_more</span>
                            </button>
                    ...
                    @if ($product->natural_benefits)
                        <div class="border-b border-outline-variant/10 pb-4">
                            <button @click="activeTab = activeTab === 'benefits' ? null : 'benefits'"
                                ...
                                <span class="material-symbols-outlined text-[16px] transition-transform"
                                    :class="activeTab === 'benefits' ? 'rotate-180 text-primary' : ''">expand_more</span>
                            </button>
                    ...
                    @if (is_array($product->nutritional_values) && count($product->nutritional_values) > 0)
                        <div class="border-b border-outline-variant/10 pb-4">
                            <button @click="activeTab = activeTab === 'nutri' ? null : 'nutri'"
                                ...
                                <span class="material-symbols-outlined text-[16px] transition-transform"
                                    :class="activeTab === 'nutri' ? 'rotate-180 text-primary' : ''">expand_more</span>
                            </button>
                            <div x-show="activeTab === 'nutri'" x-collapse x-cloak
        ...
        @if ($relatedProducts->isNotEmpty())
            <section class="mt-32 pt-16 border-t border-outline-variant/10">
                <h3 class="font-headline text-2xl font-bold mb-12 text-center">
                    {{ app()->getLocale() == 'es' ? 'Te Podría Interesar' : 'You May Also Like' }}</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    @foreach ($relatedProducts as $rel)
                        ...
                        @endphp
                        <div
                            class="group flex flex-col justify-between bg-surface-container border border-outline-variant/5 p-4 hover:border-outline-variant/20 transition-all duration-300">
                            <div>
                            ...
                            <div
                                class="mt-4 flex justify-between items-center pt-2 border-t border-outline-variant/10 text-xs">
                                @if ($relVariant)
                                    <span class="text-secondary font-bold">S/
                                        {{ number_format($relVariant->price, 2) }}</span>
                                @else
